...
void DE_stage() {
  ...
  int iii;
  for (iii = 0; iii< NUM_CONTROL_STORE_BITS; ++iii)
  {
    DE_CS[iii] = CONTROL_STORE[CONTROL_STORE_ADDRESS][iii];
  }
  ...
  printf("%d\n", Get_DE_BR_STALL(DE_CS));
   ...
   if (PS.DE_V == 1)
   {
    if (Get_DE_BR_OP(DE_CS) == 1 & (v_sr_ld_cc | v_mem_ld_cc | v_agex_ld_cc) == 1)
      dep_stall = 1;
    else if (v_sr_ld_reg == 1)
    {
      if ((PS.SR_DRID == sr1 & Get_SR1_NEEDED(DE_CS) == 1) | (PS.SR_DRID == sr2 & Get_SR2_NEEDED(DE_CS) == 1))
        dep_stall = 1;
    }
...
void FETCH_stage() {
  ...
  if (LD_DE)
  {
    NEW_PS.DE_NPC = npc;
    NEW_PS.DE_IR = output;
    if (v_mem_br_stall == 0 & v_agex_br_stall == 0 & v_de_br_stall == 0 & icache_r == 1)
      NEW_PS.DE_V = 1;
    else
      NEW_PS.DE_V = 0;
  }
