...
saxpyCuda(long total_elems, float alpha, float* xarray, float* yarray, float* resultarray, int partitions) {
    ...
    //
    int *x_dev, *y_dev, *result_dev;
    cudaMalloc(&x_dev, total_elems * sizeof(float));
    cudaMalloc(&y_dev, total_elems * sizeof(float));
    cudaMalloc(&result_dev, total_elems * sizeof(float)); 
    ...
    //
    cudaMemcpy(x_dev, xarray, total_elems*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(y_dev, yarray, total_elems*sizeof(float), cudaMemcpyHostToDevice);
    ...
    saxpy_kernel<<<Block_num, threadsPerBlock>>>(total_elems, alpha, x_dev, y_dev, result_dev);
    ...
    //
    cudaMemcpy(resultarray, result_dev, total_elems * sizeof(float), cudaMemcpyDeviceToHost);
    ...
    //
    freeArrays(x_dev, y_dev, result_dev);
