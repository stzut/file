...
saxpyCuda(long total_elems, float alpha, float* xarray, float* yarray, float* resultarray, int partitions) {
    ...
    for (int i=0; i<partitions; i++) {
        ...
        //
        cudaMemcpyAsync(device_x[offset], xarray[offset], streamSize*sizeof(float), cudaMemcpyHostToDevice, cudaMemcpyHostToDevice, stream[i]);
        cudaMemcpyAsync(device_y[offset], yarray[offset], streamSize*sizeof(float), cudaMemcpyHostToDevice, cudaMemcpyHostToDevice, stream[i]);
        //
        ...
        int Block_num = getBlocks(total_elems, threadsPerBlock) / partitions;
        // run saxpy_kernel on the GPU
        saxpy_kernel<<<Block_num, threadsPerBlock, 0, stream[i]>>>(total_elems, alpha, device_x, device_y, device_result);
        ...
        //
        cudaMemcpyAsync(resultarray[offset], device_result[offset], streamSize * sizeof(float), cudaMemcpyDeviceToHost, cudaMemcpyDeviceToHost, stream[i]);
